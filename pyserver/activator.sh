...
while true; do
    ...
    if [ "$userInput" == "activate" ]; then
        ...
        "$PYTHON_EXECUTABLE" -m pip install -r /Users/sagarpoudel/Downloads/transfer/pyserver/fast_api_app/requirements.txt
        if [ $? -eq 0 ]; then
        ...
        "$PYTHON_EXECUTABLE" /Users/sagarpoudel/Downloads/transfer/pyserver/fast_api_app/server.py
    else
        eval "$userInput"
    fi
done
